...
exports.jsConcat = function(file, cb, options){
  ...
  var rv = meta.dependencies.map(function(dep){
    if (dep.charAt(0) === '.') {
      ...
      if (_.contains(records, id)) return '';
      ...
      var astCache = ast.getAst(fs.readFileSync(fpath, 'utf8')),
          srcId = ast.parseFirst(astCache).id;

      astCache = ast.modify(astCache, function(v){
        //make id absolute
        if (v.charAt(0) === '.') {
          console.info(srcId, v);
